...
public class PartitionForeachCollectorTest {
    ...
    public void all_partitions_equal() {
        ...
        IntStream.rangeClosed(1, 9).boxed().collect(Collectors.batchingForeach(3, result::add));
    ...
    public void last_partition_smaller() {
        List<List<Integer>> result = new ArrayList<>();
        IntStream.range(1, 9).boxed().collect(Collectors.batchingForeach(3, result::add));
    ...
    public void merge_partitions_when_parallel() {
        ...
        List<List<Integer>> result = new ArrayList<>();
        IntStream.range(1, 9).boxed().parallel().collect(Collectors.batchingForeach(3, t -> {
            synchronized (lock) {
                result.add(t);
            }
        }));
